...
type
  TMainForm = class(TForm)
    ...
    procedure ScrollContentPaint(Sender: TObject; Canvas: TCanvas;
      const ARect: TRectF);
  private const
...
begin
  ...
  Re:=TControl.Create(Self);
  Re.HitTest:=False;
  Re.Align:=TAlignLayout.None;
  Re.OnPaint:=ScrollContentPaint;
...
//  TAniCalculationsAccess(FAniCalc).StorageTime:=0.1;//1000;
  ...
  Ani.NotInverseDelay:=0.5;

  var Bitmap:=TBitmap.Create;

  Bitmap.SetSize(600,600);
  Bitmap.Canvas.BeginScene;
  Bitmap.Canvas.Fill.Kind:=TBrushKind.Solid;
  Bitmap.Canvas.Fill.Color:=MakeColor(claBlack,0.5);
  Bitmap.Canvas.FillEllipse(Bitmap.BoundsF,1);
  ...
  AniStyle:=TActiveStyleObject.Create(Self);
  AniStyle.Align:=TAlignLayout.Contents;
  AniStyle.TouchAnimation.Link.Add;
  AniStyle.TouchAnimation.Link.Links[0].SourceRect.Rect:=Image.Bitmap.BoundsF;
  AniStyle.Source:=Image;
  AniStyle.TouchAnimation.PressingDuration:=0.5;
...
begin
  ...
  if (Re.Parent=nil) and (PressView<>nil) then
  begin
    CurrentTime:=0;
    Re.BoundsRect:=PressView.LocalRect;
  end else if CurrentTime<200 then
  begin
    CurrentTime:=CurrentTime+Timer1.Interval;
    IScene(Self).AddUpdateRect(Re.AbsoluteRect);
  end;

  Re.Parent:=PressView;

//  if Assigned(Ani.Parent) then Ani.Leave
//  else
//    Ani.Start(Views[4],PointF(100,80));

//  if Assigned(Re.Parent) then Re.Parent:=nil else Re.Parent:=Views[4];
end;
...
procedure TMainForm.ScrollContentPaint(Sender: TObject; Canvas: TCanvas;
  const ARect: TRectF);
var FProcessColor: TAlphaColor;
begin

    FProcessColor:=AnimatedInterpolateColor(claNull,MakeColor(0,0,0,160),Min(1.0,CurrentTime/200));
//    FProcessColor:=claRed;
    Canvas.Fill.Kind:=TBrushKind.Solid;
    Canvas.Fill.Color:=FProcessColor;
    Canvas.FillRect(ARect,2,2,AllCorners,Re.AbsoluteOpacity);
...
//    V.PressedPosition:=P;
//    AniStyle.Parent:=V;
//    AniStyle.TouchAnimation.StartAnimation(V,TTouchAnimationAdapter.TAnimationKind.Pressed);
