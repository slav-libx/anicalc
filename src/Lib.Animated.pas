...
begin
  ...
  try
    if ParentControl.HitTest then TControlAccess(ParentControl).Click;
...
begin
  var C:=TMouseTarget.Create(Shape,AnimationClass);
...
begin
  if Animation.Inverse then HideShape;
end;
...
begin
  ...
  begin
    ...
    if Target.Children[I] is TMouseTarget then Exit(TMouseTarget(Target.Children[I]).Animation) else
...
begin
  ...
  if not Inverse then
  begin
    FFill.Bitmap.Bitmap.SetSize(Ceil(ARect.Width),Ceil(ARect.Height));
    C:=FFill.Bitmap.Bitmap.Canvas;
    C.BeginScene;
    FProcessColor:=DrawRippleEffect(C,ARect,FPoint,FFromColor,FToColor,
...
var SaveDisableAlign: Boolean;
begin
  if (Control is TControl) and (Control<>PaintControl.Parent) then
  begin
    SaveDisableAlign:=TControlAccess(Control).FDisableAlign;
    TControlAccess(Control).FDisableAlign:=True;
    try
...
procedure TTouchAnimation.Leave;
begin
  if not Inverse then
  if PaintControl.HasParent then
  begin
    Inverse:=True;
    SetColors;
